...
var TimeContainer = /** @class */ (function (_super) {
    ...
    function TimeContainer() {
        ...
        _this.handleChange = function (item) { return function (e) {
            var _a;
            var min = 0;
            var max = item === 'starthour' || 'endhour' ? 12 : 59;
        ...
        _this.handleDown = function (item, data) { return function () {
            ...
            if (_this.props.allowedTime === true && _this.state.startsessions === "PM" && data === "startsessions" && _this.state.starthour > 4) {
                _this.setState(__assign({}, _this.state, { starthour: 4 }), function () { _this.downChangeFunction(item, min); });
            }
                ...
                if (_this.props.allowedTime === true && _this.state.startsessions === "AM" && data === "startsessions") {
                    min = 8;
                }
                else if (_this.props.allowedTime === true && _this.state.endsessions === "AM" && data === "endsessions") {
                    min = 8;
                }
        ...
        _this.handleSession = function (data, item) { return function () {
            var _a;
            _this.setState(__assign({}, _this.state, (_a = {}, _a[data] = item === 'AM' ? 'PM' : 'AM', _a)), function () { return _this.invokeOnChange(); });
        }; };
        ...
        _this.invokeOnChange = function () {
            ...
            var starttime = starthour + ":" + startminute + " " + startsessions;
            var endtime = endhour + ":" + endminute + " " + endsessions;
            FunctionUtil_1.ifExistCall(onChange, starttime, endtime);
        };
        _this.onNowPress = function (data) {
            if (data === "startsessions") {
                _this.setState(__assign({}, _this.state, { starthour: new Date().getHours() % 12 || 12, startminute: new Date().getMinutes() || 0, startsessions: new Date().getHours() >= 12 ? 'PM' : 'AM' }), function () { return _this.invokeOnChange(); });
            }
            else {
                _this.setState(__assign({}, _this.state, { endhour: new Date().getHours() % 12 || 12, endminute: new Date().getMinutes() || 0, endsessions: new Date().getHours() >= 12 ? 'PM' : 'AM' }), function () { return _this.invokeOnChange(); });
            }
        };
        _this.onTimePress = function (data) {
            if (data === "startsessions") {
                _this.setState(__assign({}, _this.state, { starthour: 12, startminute: 0, startsessions: 'AM' }), function () { return _this.invokeOnChange(); });
            }
            else {
                _this.setState(__assign({}, _this.state, { endhour: 12, endminute: 0, endsessions: 'AM' }), function () { return _this.invokeOnChange(); });
            }
        };
        return _this;
    }
